...
def confirm_order(update: Update, context: CallbackContext):
    ...
    if not is_admin(user_id):
        ...
        context.bot.send_message(chat_id=original_chat_id, text=f'Заказ от пользователя {username} ({telegram_id}):\n{order_text}', reply_markup=reply_markup)
        query.edit_message_text(f'Ваш заказ успешно размещен. Администраторы увидят его и свяжутся с вами при необходимости.')
        return
    ...
    keyboard = [[InlineKeyboardButton("Да", callback_data='yes'),
                ...
                [InlineKeyboardButton("Забанить пользователя", callback_data='ban'),
                 InlineKeyboardButton("Удалить заказ", callback_data='delete')]]
    reply_markup = InlineKeyboardMarkup(keyboard)

    # Отправка заказа только в тот чат, в котором было отправлено исходное сообщение
    context.bot.send_message(chat_id='-4080817111', text=f'Заказ от пользователя {username} ({telegram_id}):\n{order_text}', reply_markup=reply_markup)
